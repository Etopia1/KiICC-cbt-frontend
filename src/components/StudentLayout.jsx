...
import {
    ...
    Bell, LogOut, Menu, X, ChevronRight, GraduationCap, Activity
} from 'lucide-react';
...
const StudentLayout = ({ children }) => {
    ...
    const navLinks = [
        { path: '/student/dashboard', label: 'Main Portal', icon: LayoutDashboard },
        { path: '/student/exams', label: 'Availability', icon: BookOpen },
        { path: '/student/results', label: 'Registry Logs', icon: ClipboardCheck },
        { path: '/student/profile', label: 'Candidate ID', icon: User },
    ];
    ...
    const currentLabel = navLinks.find(i => location.pathname === i.path)?.label || 'Portal Node';
                ...
                <div className="p-8 border-b border-slate-50 relative group">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                            ...
                            <span className="text-[9px] font-black text-[#c5a059] uppercase tracking-[0.3em]">Candidate Node</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 px-2">
                        <div className="w-10 h-10 rounded-2xl bg-white border border-slate-100 flex items-center justify-center shrink-0 shadow-sm relative group cursor-pointer transition-transform hover:scale-110">
                            {user?.profilePicture ? (
                                ...
                                <span className="text-[12px] font-black text-[#c5a059] uppercase italic">{user?.fullName?.charAt(0) || 'S'}</span>
                            )}
                            <div className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-500 border-2 border-white rounded-full shadow-sm" />
                        </div>
                        <div className="min-w-0">
                            <p className="text-[12px] font-black text-[#1a150e] truncate uppercase italic leading-none mb-1.5">{user?.fullName || 'Candidate'}</p>
                ...
                <header className="sticky top-0 z-30 border-b border-slate-50 bg-white/70 backdrop-blur-2xl px-8 py-5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden sm:flex flex-col items-end">
                             <p className="text-[11px] font-black text-[#1a150e] leading-none uppercase italic">{user?.info?.classLevel || 'Class'}</p>
                             <p className="text-[8px] font-bold text-[#c5a059] uppercase tracking-[0.2em] mt-1">Registry Level</p>
                        </div>
